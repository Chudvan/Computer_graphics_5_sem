...
public:
	...
	operator Vector4<TYPE>() const{ return Vector4<TYPE>(x, y, z, 1); }
	operator Point<TYPE>() { return Point<TYPE>(x, y); }
...
template <class TYPE>
Vector<TYPE> operator*(const TYPE& k) { return Vector<TYPE>(x * k, y * k, z * k); }
...
template<class TYPE>
Vector<TYPE> operator^(const Vector<TYPE>& l, const Vector<TYPE>& r)
{
